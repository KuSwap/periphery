...
import './interfaces/IKuswapFactory.sol';
import './libraries/TransferHelper.sol';

import './libraries/KuswapLibrary.sol';
import './interfaces/IKuswapRouter01.sol';
import './interfaces/IERC20.sol';
import './interfaces/IWETH.sol';

contract KuswapRouter01 is IKuswapRouter01 {
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) private {
        for (uint i; i < path.length - 1; i++) {
            ...
            IKuswapPair(KuswapLibrary.pairFor(factory, input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(path[0], msg.sender, KuswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut, uint swapFee) public pure override returns (uint amountIn) {
        return KuswapLibrary.getAmountOut(amountOut, reserveIn, reserveOut, swapFee);
    }
